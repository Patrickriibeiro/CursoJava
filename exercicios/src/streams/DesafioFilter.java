...
public class DesafioFilter {
	public static void main(String[] args) {
		...
		Produto p2 = new Produto ("Notebook" ,4899 , 0.32 , 30);
		...
		Predicate<Produto> produtoRelevate = p -> p.preco >= 500; 
		
		Function<Produto , String> chamadaPromocional = p -> "Aproveite" +
		p.nome + " por R$" + p.preco;
		
		produtos.stream()
		.filter(superPromocao)
		.filter(freteGratis)
		.filter(produtoRelevate)
